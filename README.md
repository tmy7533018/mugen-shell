...
- 日常のためのパネルが揃っています。カレンダー、タイマー、天気、音楽、クリップボード、通知、アプリランチャー、スクリーンショット
- 壁紙は画像・動画に対応しており、滑らかなトランジションで切り替わります。
- 壁紙から生成した色を、GTK アプリや Hyprland まで含めたデスクトップ全体に配色します。バーやパネルの面は 5 つのパレットから選べます。ライトとダークの切り替えも、デスクトップ全体に効きます。
- ひととおりのシステム操作ができます。オーディオ、バックライト、WiFi、Bluetooth、IME、バッテリー、システムトレイ
- ロック画面もシェルの一部です。時計、カレンダー、再生中の曲、天気などを並べたグリッドデザインで、外部のロッカーは使いません。
- デスクトップのアシスタント Yura とチャットできます。返事の読み上げもできます (オプション)。
- 全体がなめらかなアニメーションで動きます。アニメーションは簡単なカスタマイズができます。
- Settings ウィンドウでいろいろな設定を変更できます。バーは高さ・角丸のほか、面の色そのものも指定できます。
