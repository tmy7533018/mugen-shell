...
Rectangle {
    ...
    RowLayout {
        ...
        Common.Slider {
            ...
            to: 30
            stepSize: 1
            value: section.settingsManager ? Math.round(section.settingsManager.autoCloseTimerInterval / 1000) : 5
            display: value === 0 ? "Off" : (Math.round(value) + "s")

            onMoved: nv => {
                if (section.settingsManager) section.settingsManager.autoCloseTimerInterval = Math.round(nv) * 1000
                section.bump()
            }
            onReleased: {
