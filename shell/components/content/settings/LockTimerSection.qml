...
Rectangle {
    ...
    RowLayout {
        ...
        Common.Slider {
            ...
            to: 30
            stepSize: 1
            value: section.settingsManager ? section.settingsManager.lockTimerMinutes : 10
            display: Math.round(value) + "m"

            onMoved: nv => {
                if (section.settingsManager) section.settingsManager.lockTimerMinutes = Math.round(nv)
                section.bump()
            }
            onReleased: {
