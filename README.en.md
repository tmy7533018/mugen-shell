...
Install paths, dependencies, keybindings, and configuration all live in [SETUP.en.md](SETUP.en.md); `Super + /` also brings up the shortcut reference in a running shell.
...
Voice input is optional. Hold `Super + Z`, talk, and the reply comes back spoken; the voice that ships is Japanese and reads whatever language it is given, until you hand a language its own voice in Settings. Everything is configured under **Settings → AI / Yura**, and [SETUP.en.md](SETUP.en.md#configuring-mugen-ai) covers the rest.
