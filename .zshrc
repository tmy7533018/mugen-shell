...
ascii_fetch() {
  ...
  local tmpfile
  tmpfile="$(mktemp /tmp/ff_color.XXXXXX)"
  command fastfetch --logo none --pipe false > "$tmpfile"

  local ascii
  ascii="$(jp2a --width="$width" --colors "$img")" || {
    tput cnorm
    command fastfetch
    rm -f "$tmpfile"
    return
  ...
  local IFS=$'\n'
  local ascii_lines=("${(@f)${ascii}}")
  local info_lines=("${(@f)$(<"$tmpfile")}")
  local ascii_h=${#ascii_lines[@]}
  local info_h=${#info_lines[@]}

  # Left side: ASCII art
  local start_y=$((1 + y_offset))
  local i
  for i in {1..$ascii_h}; do
    printf '\e[%d;%dH%s' $((start_y + i - 1)) $((1 + left_margin)) "${ascii_lines[i]}"
  done

  # Right side: fastfetch output
  local start_x=$((width + gap + left_margin))
  for i in {1..$info_h}; do
    printf '\e[%d;%dH%s' "$i" "$start_x" "${info_lines[i]}"
  done

  # Move cursor to bottom and add newline
  local max_h=$(( ascii_h + y_offset > info_h ? ascii_h + y_offset : info_h ))
  printf '\e[%d;1H\n' "$((max_h + 1))"
