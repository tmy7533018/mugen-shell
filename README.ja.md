...
- 音声入力 (オプション)。**「Hey Yura」**と呼びかけて話すだけ (VOICEVOX 生成の日本語音声で訓練した自作 openWakeWord モデル)。whisper.cpp が文字起こしし、返答は同じ会話スタックを通って VOICEVOX が文単位で読み上げ、bar とパネルにもライブでミラーされます。両 UI に push-to-talk のマイクボタンが付き、listening 中はキャンセルボタンに変わります。セットアップは [SETUP.ja.md → 音声入力](SETUP.ja.md#音声入力-オプション)。
